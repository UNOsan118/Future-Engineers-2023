...
while True:
        ...
        if red_ratio > green_ratio:
            ...
            if green_ratio < 0.4:
                ...
                if center_ratio_green > 0.85:
                    ...
                    if green_ratio > 0.015:
                        steer = -(green_ratio / 0.015) * 70
                    if steer > 0 and green_ratio > 0.01:
                        steer = 0
                    elif green_ratio > 0.006:
                        steer = steer * 2
    ...
    if rotation_mode == "blue":
        over_sign = 0
        if red_ratio > green_ratio: # 赤のほうが近い、次のセクションの標識を面積で弾く
            if black_left_ratio >= 0.004: # 何も標識が見えていないときは壁が近いかどうかで進む量を変える
                over_sign = 3
                if center_red_y < blue_center_y and red_ratio >= 0.0025: # 標識が見えていたら標識を優先する
                    over_sign = 1
            elif center_red_y < blue_center_y and red_ratio >= 0.0025: # 壁に近くなくても標識を拾う
                    over_sign = 1
            else:
                over_sign = 0
                if center_green_y < blue_center_y and green_ratio >= 0.0025: # 赤のほうが近くても線を超えていなかったら奥の緑を見落としてしまう
                    over_sign = 2

        else: # 緑のほうが近い
            if black_left_ratio >= 0.004:
                over_sign = 3
                if center_green_y < blue_center_y and green_ratio >= 0.0025:
                    over_sign = 2
            elif center_green_y < blue_center_y and green_ratio >= 0.0025:
                over_sign = 2
            else:
                over_sign = 0
                if center_red_y < blue_center_y and red_ratio >= 0.0025: # 緑のほうが近くても線を超えていなかったら奥の赤を見落としてしまう
                    over_sign = 1

    elif rotation_mode == "orange":
        over_sign = 0
        if red_ratio > green_ratio:
            if black_right_ratio >= 0.004:
                over_sign = 3
                if center_red_y < orange_center_y and red_ratio >= 0.0025:
                    over_sign = 1

            elif center_red_y < orange_center_y and red_ratio >= 0.0025:
                    over_sign = 1
            else:
                over_sign = 0
                if center_green_y < orange_center_y and green_ratio >= 0.0025:
                    over_sign = 2
        else:
            if black_right_ratio >= 0.004:
                over_sign = 3
                if center_green_y < orange_center_y and green_ratio >= 0.0025:
                    over_sign = 2
            elif center_green_y < orange_center_y and green_ratio >= 0.0025:
                    over_sign = 2
            else:
                over_sign = 0
                if center_red_y < orange_center_y and red_ratio >= 0.0025:
    ...
    cmd = "{:4d},{:3d},{},{},{}@".format(steer_int, speed, sign_flag, rmode, over_sign)
