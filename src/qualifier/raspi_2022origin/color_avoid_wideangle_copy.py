...
while True:
    ...
    # 黒色の比の大きさでどこの壁なのかを特定
    if black_right_ratio > 0.8 and black_left_ratio > 0.8:
        wall_right = True
        wall_left = True
    elif black_right_ratio > 0.6 and black_left_ratio > 0.6:
        if black_right_ratio > black_left_ratio:
            wall_right = True
        else:
            wall_left = True
    else:
        if black_right_ratio > 0.55:
            wall_right = True
        elif black_left_ratio > 0.55:
            wall_left = True
    ...
    if red_ratio >= 0.0025 or green_ratio >= 0.0025:
        ...
        if red_ratio > green_ratio:
            if red_ratio < 0.4:
                ...
                # 右に曲がりたい時に右壁が近くなってきた場合左に少し避ける処理
    ...
    # 左回りの時で奥に赤の標識が見えている時の処理
    if rotation_mode == "blue":
        if red_ratio > 0.001 and center_red_x/width < 0.8  and  center_red_x/width > 0.1 and ok_blue:
            force_sign = 1

    # 右回りの時で奥に緑の標識が見えている時の処理
    elif rotation_mode == "orange":
        if green_ratio > 0.001 and center_green_x / width > 0.2 and center_green_x / width < 0.9 and ok_orange:
            force_sign = 2
